...
exports.addStudent = async function (req, res, next) {
    ...
    let result = await mysql.query('INSERT INTO sinhvien(msv, ho_ten, lop, dia_chi, so_dien_thoai, ngay_sinh, image) VALUES (?,?,?,?,?,?,?)', [msv, ho_ten, lop, dia_chi, sdt, ngay_sinh, filename]);
    res.json({
        status: 200,
        data: result
    });
...
exports.getStudentById = async function (req, res, next) {
    ...
    let sv = await mysql.query('SELECT * FROM sinhvien WHERE id = ? LIMIT 1', [id]);
    if (sv.length) {
        sv = sv[0];
        sv.image = readFile(`./image/${sv.image}.txt`);
    }
...
exports.deleteStudent = async function (req, res, next) {
    let id = req.params.id;
    await mysql.query('DELETE FROM sinhvien WHERE id = ?', [id]);
    res.json({
        status: 200,
        data: []
    });
